...
@AGENT_REGISTRY.decorator("representation_agent_ts")
class RepresentationAgentTS(BaseAgent):
	"""Representation agent for raw time series data (TS view)."""
	...
	def _run_impl(
		...
		if input_data.representation_type != RepresentationType.TS:
			raise ValueError(f"Expected TS representation, got {input_data.representation_type}")

		rep_component = RawSeriesRepresentation(name=self.name)
		...
		for channel in channels:
			...
			rep_input = RepInput(
				...
				metadata={"source": "channel_decomposer"},
			)
		...
		self.log_info(
			context,
			"RepresentationAgentTS '%s': created representations for %d channels",
			self.name,
		...
		return RepresentationOutput(
			...
			metadata={
				...
				"representation_type": RepresentationType.TS.value,
			},
		)
