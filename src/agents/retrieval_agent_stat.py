...
@AGENT_REGISTRY.decorator("retrieval_agent_stat")
class RetrievalAgentStat(BaseAgent):
	"""Retrieval agent using statistical features (STATISTIC view)."""
	...
	def _run_impl(
		...
		self.log_info(
			context,
			"RetrievalAgentStat '%s': retrieved results for %d channels",
			self.name,
			len(retrieved_sets),
		)
