...
@AGENT_REGISTRY.decorator("retrieval_agent_ts")
class RetrievalAgentTS(BaseAgent):
	"""Retrieval agent using raw time series (TS view)."""
	...
	def _run_impl(
		...
		self.log_info(
			context,
			"RetrievalAgentTS '%s': retrieved results for %d channels",
			self.name,
			len(retrieved_sets),
		)
